        ...
        protected override DataTable ProcessResult(PgConnection connection, DataTable schema)
        {
            ...
            foreach (DataRow column in schema.Rows)
            {
                ...
                switch (column["DATA_TYPE"].ToString())
                {
                    ...
                    default:
                        if (this.Connection.InternalConnection.Database.DataTypes.IndexOf(column["DATA_TYPE"].ToString()) != -1)
                        {
                            column["COLUMN_SIZE"] = this.Connection.InternalConnection.Database.DataTypes[column["DATA_TYPE"].ToString()].Size;
                        }
                        break;
                }
            }
